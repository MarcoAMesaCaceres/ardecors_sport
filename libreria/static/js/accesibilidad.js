document.addEventListener('DOMContentLoaded', function() {
  ...
  toggleZoomIn.addEventListener('click', function() {
      fontSize = Math.min(maxFontSize, fontSize + 2); // Prevent font size from exceeding maxFontSize
      updateFontSize();
  });

  // Decrease font size
  toggleZoomOut.addEventListener('click', function() {
      fontSize = Math.max(minFontSize, fontSize - 2); // Prevent font size from going below minFontSize
      updateFontSize();
  });
